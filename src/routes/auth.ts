...
const signUpSchema = z.object({
    ...
    deliveryCost: z.number().optional(), // Nuevo campo opcional
});
...
export const authRoute = new Hono()
    .post('/register', zValidator("json", signUpSchema), async (c) => {
        const { email, password, shopname, username, address, category, includeDelivery, deliveryCost } = c.req.valid("json");
    ...
    .put('/update', zValidator("json", updateSchema), async (c) => {
        try {
            const { id, shopname, username, address, imageBase64, businessHours, paymentMethod, whatsappNumber, includeDelivery, deliveryCost } = c.req.valid("json");
            const db = drizzle(pool);
            ...
            const updateData: Partial<typeof users.$inferInsert> = {
                ...
                ...(includeDelivery !== undefined && { includeDelivery }),
                ...(deliveryCost !== undefined && { deliveryCost }), // Nuevo campo
            };
            ...
            return c.json({
                ...
                user: { ...updateData, profileImageURL }
            }, 200);
        } catch (error: any) {
    ...
    .get('/get/:username', async (c) => {
        ...
        try {
            const user = await db.select({
                ...
                includeDelivery: users?.includeDelivery,
                deliveryCost: users?.deliveryCost // Nuevo campo
            }).from(users)
                .where(eq(users.username, username));
                ...
                if (currentDate > cutoffDate) {
                    ...
                    return c.json({ 
                        user: {
                            ...
                            includeDelivery: user[0].includeDelivery,
                            deliveryCost: user[0].deliveryCost // Nuevo campo
                        }, 
                        showPage: true 
                    }, 200);
                }
