...
productsRoute.post("/kid-image/:id", authMiddleware, zValidator("json", personalizeSchema), async (c) => {
  const id = Number(c.req.param("id"));
  const db = drizzle(pool);
  ...
  try {
    ...
    if (!product.imageURL) {
      return c.json(
        { message: "El producto no tiene una imagen para generar vista de niño." },
        { status: 400 }
      );
    }
    ...
    console.log(`Enviando solicitud al worker para generar vista de niño del producto ID: ${id}`);
    const workerResponse = await requestQueue.enqueue(workerPayload, workerUrl);
    console.log(`Respuesta del worker recibida correctamente`);
    ...
    if (imagePart?.inlineData) {
      const generatedImageData = imagePart.inlineData.data;
      const mimeType = imagePart.inlineData.mimeType || "image/png";
      const kidImageUrl = await saveImage(`data:${mimeType};base64,${generatedImageData}`);
      console.log("Imagen de niño guardada en:", kidImageUrl);
    ...
    console.error("Error en la ruta /kid-image:", error);
    const errorMessage = error.message || "Error interno del servidor al generar la imagen de niño.";
    return c.json({ message: errorMessage }, { status: 500 });
  }
});

productsRoute.post("/outfit-image/:id", authMiddleware, zValidator("json", personalizeSchema), async (c) => {
  const id = Number(c.req.param("id"));
  const db = drizzle(pool);
  const workerUrl = "https://personalized-worker.facucordoba200.workers.dev/outfit-image";
  const requestQueue = GeminiRequestQueue.getInstance();
  let token = getCookie(c, 'token');
  if (!token) {
    // Buscar en el header Authorization
  ...
  try {
    ...
    if (!product.imageURL) {
      return c.json(
        { message: "El producto no tiene una imagen para generar vista de conjunto." },
        { status: 400 }
      );
    }
    ...
    console.log(`Enviando solicitud al worker para generar vista de conjunto del producto ID: ${id}`);
    const workerResponse = await requestQueue.enqueue(workerPayload, workerUrl);
    console.log(`Respuesta del worker recibida correctamente`);
    ...
    if (imagePart?.inlineData) {
      const generatedImageData = imagePart.inlineData.data;
      const mimeType = imagePart.inlineData.mimeType || "image/png";
      const outfitImageUrl = await saveImage(`data:${mimeType};base64,${generatedImageData}`);
      console.log("Imagen de conjunto guardada en:", outfitImageUrl);
      ...
      console.log(`Registro insertado en tabla 'images' para producto ${id}, URL: ${outfitImageUrl}`);
      ...
      return c.json({
        message: "Imagen de conjunto generada correctamente.",
    ...
    console.error("Error en la ruta /outfit-image:", error);
    const errorMessage = error.message || "Error interno del servidor al generar la imagen de conjunto.";
    return c.json({ message: errorMessage }, { status: 500 });
  }
});

productsRoute.post("/girl-kid-image/:id", authMiddleware, zValidator("json", personalizeSchema), async (c) => {
  const id = Number(c.req.param("id"));
  const db = drizzle(pool);
  const workerUrl = "https://personalized-worker.facucordoba200.workers.dev/girl-kid-image";
  const requestQueue = GeminiRequestQueue.getInstance();
  let token = getCookie(c, 'token');
  if (!token) {
    // Buscar en el header Authorization
  ...
  try {
    ...
    if (!product.imageURL) {
      return c.json(
        { message: "El producto no tiene una imagen para generar vista de niña." },
        { status: 400 }
      );
    }
    ...
    console.log(`Enviando solicitud al worker para generar vista de niña del producto ID: ${id}`);
    const workerResponse = await requestQueue.enqueue(workerPayload, workerUrl);
    console.log(`Respuesta del worker recibida correctamente`);
    ...
    if (imagePart?.inlineData) {
      const generatedImageData = imagePart.inlineData.data;
      const mimeType = imagePart.inlineData.mimeType || "image/png";
      const girlKidImageUrl = await saveImage(`data:${mimeType};base64,${generatedImageData}`);
      console.log("Imagen de niña guardada en:", girlKidImageUrl);
      ...
      console.log(`Registro insertado en tabla 'images' para producto ${id}, URL: ${girlKidImageUrl}`);
      ...
      return c.json({
        message: "Imagen de niña generada correctamente.",
        girlKidImageUrl: girlKidImageUrl,
        imageId: result[0].id
      }, { status: 200 });
    } else {
      console.warn("No se encontró inlineData en la respuesta del worker:", workerResponse);
