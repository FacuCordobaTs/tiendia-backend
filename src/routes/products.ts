...
productsRoute.post("/kid-image-girl/:id", authMiddleware, zValidator("json", personalizeSchema), async (c) => {
  const id = Number(c.req.param("id"));
  const db = drizzle(pool);
  const workerUrl = "https://personalized-worker.facucordoba200.workers.dev/kid-image-girl";
  const requestQueue = GeminiRequestQueue.getInstance();
  const token = getCookie(c, 'token');
  if (!token) return c.json({ error: 'Unauthorized' }, 401);
