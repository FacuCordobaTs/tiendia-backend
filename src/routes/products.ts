...
import UUID from 'uuid-js'; // Generador de UUID
...
// Función para eliminar la imagen local
...
const productSchema = z.object({
  ...
  username: z.string(),
  sizes: z.array(z.string()).optional() // Campo para tallas
});
...
export const productsRoute = new Hono()
  .post('/create', authMiddleware, zValidator("json", productSchema), async (c) => {
    const { name, price, description, image, username, sizes } = c.req.valid("json");
    ...
    try {
      ...
      await db.insert(products).values({
        ...
        description,
        imageURL: imageUrl,
        sizes: sizes ? JSON.stringify(sizes) : null, // Convertir array a JSON
        createdAt: new Date(),
        createdBy: username
      })
  ...
  .get('/list/:username', async (c) => {
    ...
    try {
      const productsListed = await db.select({
        id: products.id,
        name: products.name,
        price: products.price,
        description: products.description,
        imageURL: products.imageURL,
        sizes: products.sizes, // Agregar tallas
        createdAt: products.createdAt
      }).from(products).where(eq(products.createdBy, username));
  ...
  .get('/get/:id', async (c) => {
    ...
    try {
      const product = await db.select({
        ...
        description: products.description,
        imageURL: products.imageURL,
        sizes: products.sizes, // Campo JSON
        createdAt: products.createdAt
      ...
      // Manejar sizes sin necesidad de JSON.parse si ya es un objeto
      const parsedProduct = {
        ...product[0],
        sizes: product[0].sizes || [] // Si sizes es null/undefined, asigna un array vacío
      };
  ...
  .put('/update', zValidator("json", updateProductSchema), async (c) => {
    try {
      const { name, price, description, imageBase64, id, sizes } = c.req.valid("json");
      const db = drizzle(pool);

      let imageURL: string | undefined;

      // Procesar imagen si se provee
      if (imageBase64) {
  ...
  .delete('/delete/:id', authMiddleware, async (c) => {
    ...
    const id = Number(c.req.param('id'));
    try {
      // Obtener el producto primero para obtener la imagen
      const product = await db.select()
        .from(products)
        .where(eq(products.id, id));
